...
<%
	...
	User person = userService.findUser(userId);
	User user = null;

	out.print(userId);

	if (userPw.equals(person.getUserPw())) {
	} else {
	...
	if (userGender.equals(person.getUserGender())) {
	} else {
	...
	if (Date.valueOf(userBirthDate).equals(person.getUserBirthDate())) {
	} else {
		user = new User(userId, "", "", "", Date.valueOf(userBirthDate), "", "", "");
		userService.modifyUserBirthDate(user);
		url += "userBirthDateChange=true&";
		out.println("생년월일변경");
	}

	if (userPhoneNumber.equals(person.getUserPhoneNumber())) {
	...
	if (userEmail.equals(person.getUserEmail())) {
	} else {
